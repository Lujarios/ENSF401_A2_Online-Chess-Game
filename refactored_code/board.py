...
class Board:
    ...
    def __init__(self, rows, cols):
        ...
        self.player1_time = DEFAULT_TIME
        self.player2_time = DEFAULT_TIME
    ...
    def draw(self, win, color):
        if self.last_move and color == self.current_turn:
            y, x = self.last_move[0]
            y1, x1 = self.last_move[1]

            xx = (4 - x) +round(self.start_x + (x * self.board_rect[2] / BOARD_SIZE))
            yy = 3 + round(self.start_y+ (y * self.board_rect[3] / BOARD_SIZE))
            pygame.draw.circle(win, (0,0,255), (xx+32, yy+30), HIGHLIGHT_RADIUS, HIGHLIGHT_THICKNESS)
            xx1 = (4 - x) + round(self.start_x + (x1 * self.board_rect[2] / BOARD_SIZE))
            yy1 = 3+ round(self.start_y+ (y1 * self.board_rect[3] / BOARD_SIZE))
            pygame.draw.circle(win, (0, 0, 255), (xx1 + 32, yy1 + 30), HIGHLIGHT_RADIUS, HIGHLIGHT_THICKNESS)
    ...
    def move(self, start, end, color):
        ...
        if changed:
            ...
            if self.current_turn == "w":
                self.stored_time1 += (time.time() - self.start_time)
            else:
                self.stored_time2 += (time.time() - self.start_time)
            self.start_time = time.time()
